...
void JumpBehaviour::Update(float deltaTime) 
{
	...
	if (InputEngine::GetKeyState(GLFW_KEY_W) == ButtonState::Pressed) 
	{
		_body->GetGameObject()->SetPostion(_body->GetGameObject()->GetPosition() + glm::vec3(0.f, 1.f, 0.f));
		Gameplay::IComponent::Sptr ptr = Panel.lock();
		if (ptr != nullptr) {
			ptr->IsEnabled = !ptr->IsEnabled;
		}
	...
	if (InputEngine::GetKeyState(GLFW_KEY_S) == ButtonState::Pressed)
	{
		_body->GetGameObject()->SetPostion(_body->GetGameObject()->GetPosition() + glm::vec3(0.f, -1.f, 0.f));
		Gameplay::IComponent::Sptr ptr = Panel.lock();
		if (ptr != nullptr) {
	...
	{
		_body->GetGameObject()->SetPostion(_body->GetGameObject()->GetPosition() + glm::vec3(1.f, 0.f, 0.f));
		Gameplay::IComponent::Sptr ptr = Panel.lock();
		if (ptr != nullptr) {
			ptr->IsEnabled = !ptr->IsEnabled;
		}
